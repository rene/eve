...
from lib.silicon_id_csr import Device_Gen_Silicon_ID_CSR
...
def gen_silicon_id_pubkey_and_csr(kdk: str, sn: bytes, gen_csr: bool,
                                  csr_common_name: str, csr_org_name: str, csr_country_name: str):
    ...
    if not gen_csr:
        ...
        silicon_id_csr = Device_Gen_Silicon_ID_CSR(csr_common_name, csr_org_name, csr_country_name,
                                                   silicon_id_private_key.to_der(),
                                                   silicon_id_public_key.to_der())
        return silicon_id_public_key.to_string().hex(), silicon_id_csr.build_csr()
...
# Get the kdk string by stripping the hex prefix, check its length and validity as a hex string
# Returning empty string if the provided kdk string does not meet the standard
def get_kdk_string(kdk: str) -> str:
    kdk_str = kdk.strip()
    if kdk_str.startswith("0x") or kdk_str.startswith("0X"):
        kdk_str = kdk_str[2:]
    if len(kdk_str) != 64:
        return ""
    try:
...
def main():
    ...
    parser.add_argument("--preset", help='use the preset KDK database')
    parser.add_argument("--fixed", action='store_true', help='Use the fixed KDK value. Requires the --preset option to be set, with the first value in the file used as the fixed KDK.')
    ...
    parser.add_argument("--silicon_id_csr_cn", type=str, default="Silicon-ID-ECA", help="The common name of the Silicon ID CSR (Certificate Signing Request).")
    parser.add_argument("--silicon_id_csr_og", type=str, default="fTPM Corp", help="The organization name of the Silicon ID CSR (Certificate Signing Request).")
    parser.add_argument("--silicon_id_csr_Cn", type=str, default="US", help="The country name of the Silicon ID CSR (Certificate Signing Request).")
    ...
    if not args.destroy:
        ...
        if args.preset != None and not os.path.isfile(args.preset):
            raise ValueError("Invalid preset KDK database")
    ...
    if args.fixed == True and args.preset == None:
        raise ValueError("--fixed option must be used with the --preset option")
    preset_kdks = []
    fixed_kdk=""
    if args.preset != None:
        with open(args.preset, 'r') as file:
            preset_kdks = file.readlines()
        if args.fixed == True:
            if len(preset_kdks) < 1:
                raise ValueError("Error: incorrect number of values specified by --preset: ", len(preset_kdks))
        elif len(preset_kdks) != args.num_devices:
            raise ValueError("The number of preset KDKs is not equal to the number of devices")

        for i in range(0, len(preset_kdks)):
            device_kdk = get_kdk_string(preset_kdks[i])
            if not device_kdk:
                raise ValueError("Invalid preset KDK@" + str(i + 1) + " found: " + preset_kdks[i])
            if args.fixed == True:
                fixed_kdk = device_kdk
                break
            preset_kdks[i] = device_kdk
    ...
    if (args.prov_mode == "online"):
        # Add Silicon ID CSR DB for online provisioning mode
        silicon_id_csr_filename = 'ftpm_kdk/silicon_id_csr_db_{}-{}-{}.csv'.format(device_sn.hex()[:4], device_sn.hex()[4:], args.num_devices)
        silicon_id_csr_file = open(silicon_id_csr_filename, "w", newline='')
        silicon_id_csr_writer = csv.writer(silicon_id_csr_file,  delimiter=' ', quotechar='|', quoting=csv.QUOTE_MINIMAL)
    ...
    with open(kdkfilename, 'w', newline='') as kdkfile, open(pubkeyfilename, 'w', newline='') as pubkeyfile:
        ...
        for i in range(0, args.num_devices):
            if fixed_kdk:
                device_kdk = fixed_kdk
            elif len(preset_kdks) > 0:
                device_kdk = preset_kdks[i]
            else:
            ...
            while True:
                if (args.prov_mode == "online"):
                    silicon_id_csr_cn = '{}_{}'.format(device_sn.hex(), args.silicon_id_csr_cn)
                    device_pubkey, silicon_id_csr = gen_silicon_id_pubkey_and_csr(device_kdk, device_sn, True,
                                                                                  silicon_id_csr_cn,
                                                                                  args.silicon_id_csr_og,
                                                                                  args.silicon_id_csr_Cn)
                else:
                    device_pubkey = gen_silicon_id_pubkey_and_csr(device_kdk, device_sn, False, "", "", "")

                if len(device_pubkey) == 0:
                    print("Generate ECDSA key pair using KDK: " + device_kdk + " failed. Will start over.")
                    device_kdk = binascii.b2a_hex(bytes(32)).decode()
                else:
                    break
            ...
            if (args.prov_mode == "online"):
                silicon_id_csr_writer.writerow(["{}".format(device_sn.hex()[:4]),
                                                "{}".format(device_sn.hex()[4:]), silicon_id_csr])
