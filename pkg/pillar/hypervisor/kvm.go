...
{{- end -}}
{{end}}
{{- if or .DomainConfig.EnableVnc .HasVirtualDisplay }}
...
{{- /* With a virtual display the same device also carries the scanouts the
       host compositor puts on physical connectors: max_outputs is the number
       of connectors assigned to this domain, and xres/yres only seed the
       initial mode — the compositor pushes the real size down once it
       fullscreens the surface. */}}
{{- if ne .Machine "virt" }}
[device "video0"]
  driver = "virtio-vga"
  bus = "pcie.0"
...
{{- if .HasVirtualDisplay }}
  max_outputs = "{{.VirtualDisplayCount}}"
  edid = "on"
  xres = "{{.VirtualDisplayWidth}}"
  yres = "{{.VirtualDisplayHeight}}"
{{- if .BlobScanout }}
  blob = "on"
{{- end }}
{{- end }}
...
[device "input1"]
{{- /* An absolute pointer keeps the guest cursor in sync with the host one,
       which matters as soon as a compositor draws both. x86 already gets a
       usb-tablet above; give one to virt too when a display is attached. */}}
{{- if .HasVirtualDisplay }}
  driver = "usb-tablet"
{{- else }}
  driver = "usb-mouse"
...
type tQemuGlobalConfContext struct {
	...
	HasVirtualDisplay      bool   // true when the domain's scanouts are composited onto physical connectors by the host compositor
	VirtualDisplayCount    int    // number of virtio-gpu scanouts, one per assigned connector
	VirtualDisplayWidth    uint32 // initial mode width; the compositor overrides it once the surface is fullscreened
	VirtualDisplayHeight   uint32 // initial mode height
	BlobScanout            bool   // true when guest scanout pages are imported as dma-bufs (needs a shareable memory backend)
	types.DomainConfig
	types.DomainStatus
}
...
func (ctx KvmContext) Setup(status types.DomainStatus, config types.DomainConfig,
	...
	// A domain with connectors assigned to it drives a real monitor through
	// the host compositor instead of running headless.
	blobScanout := globalConfig != nil &&
		globalConfig.GlobalValueBool(types.DisplayBlobScanout)
	displayArgs, displayEnv := displayDeviceModelArgs(status, domainUUID, blobScanout)
	if len(displayArgs) > 0 {
		dmArgs = append(withoutHeadlessDisplay(dmArgs), displayArgs...)
	}
	...
	overhead += displayVMMOverhead(status)
	...
	if len(displayEnv) > 0 {
		spec.Get().Process.Env = append(spec.Get().Process.Env, displayEnv...)
	}
...
func (ctx KvmContext) CreateDomConfig(domainName string,
	...
	var efiDebug, dumpGuestCore, igpuNoMmap, blobScanout bool
	...
	if globalConfig != nil {
		...
		blobScanout = globalConfig.GlobalValueBool(types.DisplayBlobScanout)
	}
	...
	if len(status.VirtualDisplays) > 0 {
		qemuConfContext.HasVirtualDisplay = true
		qemuConfContext.VirtualDisplayCount = len(status.VirtualDisplays)
		qemuConfContext.VirtualDisplayWidth,
			qemuConfContext.VirtualDisplayHeight = virtualDisplayInitialMode(status.VirtualDisplays)
		qemuConfContext.BlobScanout = blobScanout
	}
