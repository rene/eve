...
The Jetson Xavier NX platform has a complex boot flow[[1]](#jetson-bootflow) that performs a lot of operations
...
1. Press F1 during initialization to enter in the bootloader setup interface. If key pressing doesn't work, perform a
   full recovery of the device[[2]](#se70-recovery)
...
1. Build an installation raw image `make ZARCH=arm64 HV=kvm PLATFORM=nvidia-nx installer-raw` (Only KVM is supported)
...
device will power off. Remove the USB Stick and power on the device again.

### Running a live image on the ThinkEdge SE70

1. Build a live raw image `make ZARCH=arm64 HV=kvm PLATFORM=nvidia-nx live-raw` (Only KVM is supported)
...
EVE should boot the live image and run from the USB Stick. **NOTE** that the NVMe must not contain an EVE installation,
otherwise it will conflict with the live image and **EVE will not run properly**.

## Jetson Xavier NX developer kit

The Jetson Xavier NX developer kit comes with e CoM (Computer on Module) without eMMC, so the entire OS should run from
an SD Card. However, the device has a small QSPI EEPROM where the bootloader can be written and
...
- Download and extract the NVIDIA Jetpack tarball from the following [link](https://developer.nvidia.com/embedded/l4t/r35_release_v1.0/release/jetson_linux_r35.1.0_aarch64.tbz2):
...
<a id="jetson-bootflow">1.</a> [Jetson Xavier NX BootFlow](https://docs.nvidia.com/jetson/archives/r35.2.1/DeveloperGuide/text/AR/BootArchitecture/JetsonXavierNxAndJetsonAgxXavierBootFlow.html)

<a id="se70-recovery">2.</a> [ThinkEdge SE70 Recovery process](https://smartsupport.lenovo.com/de/en/products/smart/smart-edge/thinkedge-se70/)
